...
def hist_is_clicked(data_selected, _, radio_button_value, scatterplot_2d_old, scatterplot_3d_old):
    ...
    if hist_type == 'categorical':
        ...
        clicked_category_bin = f"{(center_value - bin_range):.2f} - {(center_value + bin_range):.2f}"
        indices = data_selected['points'][0]['pointNumbers']
        data_aggr = range(hist_dict[feature]['data'][0]['nbinsx'])
    
    clicked_categories, selected_ids = Collection.update_filter(feature, [clicked_category, clicked_category_bin], indices)
