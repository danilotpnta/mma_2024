...
def scatterplot_2d_is_selected(
    ...
    # else:
    genre_histogram["data"][0]["marker"].update(
        {"pattern": hist_dict["genre"]["data"][0]["marker"]["pattern"]}
    )
    ...
    tempo_histogram = numerical_histogram.draw_histogram(
        "tempo", tempo_nbins, data_selected["id"]
    )
    tempo_histogram["data"][0].update(
        {"marker": hist_dict["tempo"]["data"][0]["marker"]}
    )

    loudness_nbins = hist_dict["loudness"]["data"][0]["nbinsx"]
    loudness_histogram = numerical_histogram.draw_histogram(
        "loudness", loudness_nbins, data_selected["id"]
    )
    loudness_histogram["data"][0].update(
        {"marker": hist_dict["loudness"]["data"][0]["marker"]}
    )
...
def update_selected_track_2D(
    ...
    return *update_track(
        track_id, radio_button_value, current_figure_2d, current_figure_3d
...
def update_track(track_id, radio_button_value, current_figure_2d, current_figure_3d):
    ...
    for current_figure in [current_figure_2d, current_figure_3d]:
        new_figure = go.Figure(
            data=current_figure["data"], layout=current_figure["layout"]
        )
