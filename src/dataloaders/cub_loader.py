...
def create_csv(images_dir):
    ...
    images = pandas.read_csv(
        os.path.join(TEMP_DIR, "CUB_200_2011", "images.txt"),
    ...
    for _, row in images.iterrows():
        ...
        data.append(
            [
                ...
                bird_class_id,
                bird_type,
                os.path.join(images_dir, row["image_path"]),
            ]
        )
    return pandas.DataFrame(
        data, columns=["image_id", "class_id", "class_name", "image_path"]
    )
...
def create_attr_csv():
    ...
    attribute_names = [
        name.replace("_", " ").replace("::", ": ") for name in attribute_names
    ]

    with open(
        os.path.join(
            TEMP_DIR, "CUB_200_2011", "attributes", "image_attribute_labels.txt"
        ),
        "r",
    ) as f:
        attribute_data = f.readlines()
    attribute_data = [
        [int(item) for item in line.strip().split(" ")[:4]] for line in attribute_data
    ]
