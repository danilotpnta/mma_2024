        ...
        for file in files:
            ...
            if file.endswith(".mp3"):
                ...
                wav_path = os.path.join(
                    wav_dir, os.path.splitext(relative_path)[0] + ".wav"
                )
...
def load():
    ...
    dataset_extracted = os.path.isdir(mp3_dir) and os.path.isdir(
        os.path.join(config.DATA_DIR, "fma_metadata")
    )
    ...
    if not dataset_extracted:
        ...
        for name, url in DATASET_URLS.items():
            ...
            print(f"\nExtracting data from {download_path} to {final_path}...")
            extract_dataset(download_path, config.DATA_DIR)
    ...
    if not os.path.isdir(wav_dir):
        ...
        print("Converting MP3 songs to WAV...")
        convert_all_mp3_to_wav(mp3_dir, wav_dir)

    else:
        print("Dataset already converted, skipping step...")

    print("Finished loading datasets!")


def cleanup():
    print("Perform cleanup tasks if any")
