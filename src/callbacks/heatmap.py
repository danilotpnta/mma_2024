...
def display_hover(hover_data):
    ...
    if z > 0:
        certainty = (
            "(guessing)" if z == 1 else "(probably)" if z == 2 else "(definitely)"
        )
        content.append(
            html.P(
                x + " " + certainty, style={"font-style": "italic", "font-size": "14px"}
            )
        )

    children = [html.Div(content, style={"width": "150px", "white-space": "normal"})]
...
def heatmap_is_clicked(click_data):
    ...
    class_name = (
        click_data["points"][0]["y"].split("/")[-2].split(".")[1].replace("_", " ")
    )
