...
def scatterplot_is_selected(scatterplot_fig, data_selected):
    ...
    group_by_count["total_count"] = (
        Dataset.class_count().loc[group_by_count["class_id"]].values
    )
    table_rows = group_by_count.sort_values(
        "count_in_selection", ascending=False
    ).to_dict("records")
