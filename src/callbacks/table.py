...
@callback(
    ...
    [Input("grid", "selectedRows"), Input("grid", "rowData")],
)
def table_row_is_selected(scatterplot_fig, selected_rows, added_rows):
    ...
    if selected_rows:
        ...
        graph_fig = graph.draw_graph(
            selected_rows[: config.MAX_GRAPH_NODES], valid_birds=classes_in_scatterplot
        )

    else:
        group_by_count = (
            data_selected.groupby(["class_id", "class_name"])["class_id"]
            .agg("count")
        ...
        group_by_count["count_in_selection"] = wordcloud.wordcloud_weight_rescale(
            group_by_count["count_in_selection"], 1, Dataset.class_count().max()
        )
        ...
        graph_input = [
            ...
            for class_name in data_selected.sample(
                min(config.MAX_GRAPH_NODES, len(data_selected))
            )["class_name"].values
        ]
        graph_fig = graph.draw_graph(graph_input, drag_select=True)

    sample_data = data_selected.sample(
        min(len(data_selected), config.IMAGE_GALLERY_SIZE), random_state=1
    )
    gallery_children = gallery.create_gallery_children(
        sample_data["image_path"].values, sample_data["class_name"].values
    )
