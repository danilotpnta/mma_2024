...
def draw_graph(selected_rows, valid_birds=None, drag_select=False):
    ...
    bird_classes = [
        bird_details["class_name"] for bird_details in selected_rows
    ]  # selected_rows[0]["class_name"]
    bird_species = [
        bird_class.split(" ")[-1] for bird_class in bird_classes
    ]  # bird_class.split(" ")[-1]
    ...
    node_trace = go.Scatter(
        ...
        mode="markers+text",
        hoverinfo="text",
        marker=dict(size=20, colorscale="ylgnbu", line=dict(width=0, color="darkblue")),
    )
    ...
    for node in G.nodes():
        ...
        if node in bird_classes:
            node_colours.append(
                "rgba(255, 0, 0, 0.5)" * (1 - drag_select)
                + "rgba(31, 119, 180, 0.5)" * drag_select
            )
        else:
