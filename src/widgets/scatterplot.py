...
def highlight_class_on_scatterplot(scatterplot, class_ids):
    ...
    if class_ids:
        colors = Dataset.get()["class_id"].map(
            lambda x: (
                config.SCATTERPLOT_SELECTED_COLOR
                if x in class_ids
                else config.SCATTERPLOT_COLOR
            )
        )
...
def add_images_to_scatterplot(scatterplot_fig):
    ...
    if images_in_zoom:
        for x, y, image_id in images_in_zoom:
            ...
            scatterplot_fig["layout"]["images"].append(
                dict(
                    x=x,
                    y=y,
...
def get_data_selected_on_scatterplot(scatterplot_fig):
    ...
    if "selectedpoints" in scatterplot_fig_data:
        selected_image_ids = list(
            map(
                scatterplot_fig_data["customdata"].__getitem__,
                scatterplot_fig_data["selectedpoints"],
            )
        )
