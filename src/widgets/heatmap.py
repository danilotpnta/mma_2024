...
def create_heatmap(data_selected=None):
    ...
    return html.Div(
        [
            ...
            dcc.Tooltip(id="heatmap-tooltip", loading_text="LOADING"),
        ],
        className="border-widget stretchy-widget",
...
def draw_heatmap_figure(data_selected):
    if data_selected is None or len(data_selected) == 0:
        ...
        fig.update_layout(
            ...
            margin=dict(
                b=0, l=0, r=0, t=40
            ),  # Adjust margins to ensure the text is visible
        )
    ...
    fig = px.imshow(
        attr_data.to_numpy().tolist(),
        x=attr_data.columns.tolist(),
        y=image_path,
        width=attr_data.shape[1] * 20 + 200,
        height=attr_data.shape[0] * 20 + 250 + 150,
    )

    fig.update_traces(
        dict(showscale=False, coloraxis=None, colorscale="blues"),
        hoverinfo="none",
        hovertemplate=None,
    )
    fig.update_layout(
        xaxis=dict(side="top", tickangle=280, automargin=False, fixedrange=True),
        yaxis=dict(visible=False, automargin=False, fixedrange=True),
